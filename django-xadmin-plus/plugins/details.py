            ...
            if rel_obj:
                if rel_obj.__class__ in site._registry:
                    try:
                        ...
                        has_view_perm = model_admin(self.admin_view.request).has_view_permission() or model_admin(self.admin_view.request).has_view_permission_obj(rel_obj)
                        has_change_perm = model_admin(self.admin_view.request).has_change_permission() or model_admin(self.admin_view.request).has_change_permission_obj(rel_obj)
                    except:
                ...
                try:
                    ...
                    if item_res_uri:
                        if has_change_perm:
                            ...
                            edit_url = ''
                        item.btns.append('<a data-res-uri="%s" data-edit-uri="%s" class="details-handler" rel="tooltip" title="%s"><i class="fas fa-info-circle"></i></a>'
