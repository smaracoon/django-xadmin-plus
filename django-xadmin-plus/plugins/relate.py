    ...
    def related_link(self, instance):
        ...
        for rel, view_perm, add_perm in self.get_related_list():
            ...
            link = ''.join(('<li class="with_menu_btn">',
                            ...
                            '<a href="%s?%s=%s" title="%s"><i class="icon fas fa-th-list"></i> %s</a>' %
                            (
                            ...
                            '<a><span class="text-muted"><i class="icon far fa-square"></i> %s</span></a>' % verbose_name,

                            '<a class="add_link dropdown-menu-btn" href="%s?%s=%s"><i class="icon fas fa-plus pull-right"></i></a>' %
                            (
                                reverse('%s:%s_%s_add' % (
                                    self.admin_site.app_name, label, model_name)),
        ...
        ul_html = '<ul class="dropdown-menu" role="menu">%s</ul>' % ''.join(
            links)
        return '<div class="dropdown related_menu pull-right"><a title="%s" class="relate_menu dropdown-toggle" data-toggle="dropdown"><i class="icon fas fa-list"></i></a>%s</div>' % (_('Related Objects'), ul_html)
...
class RelateObject(object):
    ...
    def get_brand_name(self):
        ...
        return mark_safe(u"<span class='rel-brand'>%s <i class='fas fa-caret-right'></i></span> %s" % (to_model_name, force_text(self.opts.verbose_name_plural)))
