...
class ReversionPlugin(BaseAdminPlugin):
    ...
    def block_top_toolbar(self, context, nodes):
        ...
        nodes.append(mark_safe('<div class="btn-group"><a class="btn btn-default btn-sm" href="%s"><i class="fas fa-trash-alt"></i> %s</a></div>' % (recoverlist_url, _(u"Recover"))))
    ...
    def block_nav_toggles(self, context, nodes):
        ...
        if obj:
            ...
            nodes.append(mark_safe('<a href="%s" class="navbar-toggle pull-right"><i class="fas fa-calendar-alt"></i></a>' % revisionlist_url))
    ...
    def block_nav_btns(self, context, nodes):
        ...
        if obj:
            revisionlist_url = self.admin_view.model_admin_url(
                'revisionlist', quote(obj.pk))
            nodes.append(mark_safe('<a href="%s" class="btn btn-default"><i class="fas fa-calendar-alt"></i> <span>%s</span></a>' % (revisionlist_url, _(u'History'))))
...
class RevisionListView(BaseReversionView):
    ...
    def get(self, request, object_id, *args, **kwargs):
        ...
        if self.has_change_permission() or self.has_change_permission_obj(self.obj):
            pass
        else:
            raise PermissionDenied
...
class ReversionAdmin(object):
    model_icon = 'fas fa-exchange-alt'
