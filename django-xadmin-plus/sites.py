...
class AdminSite(object):
    ...
    def __init__(self, name='xadmin'):
        ...
        self._registry_menuviews = []
        self._registry_customviews = []
    ...
    def copy_registry(self):
        ...
        return {
            ...
            'menuviews': copy.copy(self._registry_menuviews),
            'customviews': copy.copy(self._registry_customviews),
        }
    ...
    def restore_registry(self, data):
        ...
        self._registry_menuviews = data['menuviews']
        self._registry_customviews = data['customviews']
    ...
    def register_menuview(self, path, menu_view_class, app_label, app_title, icon, menu_label, menu_title):
        self._registry_menuviews.append({'path':path, 'menu_view_class':menu_view_class, 'app_label':app_label, 'app_title':app_title, 'icon':icon, 'menu_label':menu_label, 'menu_title':menu_title})
        from xadmin.models import MenuView
        menuview = MenuView.objects.filter(app_label=app_label, menu_label=menu_label).first()
        if not menuview:
            MenuView.objects.create(app_label=app_label, menu_label=menu_label)

    def register_customview(self, path, custom_view_class, app_label, menu_label, custom_label):
        self._registry_customviews.append({'path':path, 'custom_view_class':custom_view_class, 'app_label':app_label, 'menu_label':menu_label, 'custom_label':custom_label})
    ...
    def create_menu_view(self, menu_view_class):
        return self.get_view_class(menu_view_class).as_view()

    def create_custom_view(self, custom_view_class):
        return self.get_view_class(custom_view_class).as_view()
    ...
    def get_urls(self):
        ...
        urlpatterns += [
            re_path(
                item['path'],
                wrap(self.create_menu_view(item['menu_view_class']))
                if inspect.isclass(item['menu_view_class']) and issubclass(item['menu_view_class'], BaseAdminView)
                else include(item['menu_view_class'](self)),
                name=item['app_label'] + '_' + item['menu_label']
            )
            for item in self._registry_menuviews
        ]

        urlpatterns += [
            re_path(
                item['path'],
                wrap(self.create_custom_view(item['custom_view_class']))
                if inspect.isclass(item['custom_view_class']) and issubclass(item['custom_view_class'], BaseAdminView)
                else include(item['custom_view_class'](self)),
                name=item['app_label'] + '_' + item['menu_label'] + '_' + item['custom_label']
            )
            for item in self._registry_customviews
        ]
