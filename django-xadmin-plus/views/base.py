...
from xadmin.models import Log, MenuView
...
class BaseAdminObject(object):
    ...
    def has_model_perm(self, model, name, user=None):
        ...
        elif name == 'change' or name == 'view' or name == 'delete':
            return user.has_perm(self.get_model_perm(model, name), model)
        else:
    ...
    def get_nav_menu(self):
        ...
        def check_menu_permission(item):
            if self.user.is_superuser:
                return True
            menuview = MenuView.objects.filter(app_label=item['app_label'], menu_label=item['menu_label']).first()
            return self.user.has_perm('xadmin.visit_menuview', menuview)

        for item in self.admin_site._registry_menuviews:
            if check_menu_permission(item):
                model_dict = {
                    'title': item['menu_title'],
                    'url': '/xadmin/' + item['path'][1:-1],
                    'icon': item['icon'],
                    'perm': 'True_cs_menuview',
                    'order': 0,
                }
                ...
                if app_key in nav_menu:
                    ...
                    nav_menu[app_key] = {
                        ...
                        'menus': [model_dict],
                        'first_icon': item['icon'],
                        'first_url': '/xadmin/' + item['path'][1:-1],
                    }
    ...
    def get_context(self):
        ...
        if not settings.DEBUG and 'nav_menu' in self.request.session:
            ...
            def check_menu_permission(item):
                ...
                elif need_perm == 'True_cs_menuview':
                    return True
                elif need_perm == 'super':
                    return self.user.is_superuser
                else:
                    if self.user.has_perm(need_perm) or get_objects_for_user(self.user, need_perm):
                        return True
                    else:
                        return False
...
class CustomAdminView(CommAdminView):
    app_title = None
    menu_title = None

    @filter_hook
    def get_context(self):
        context = super(CustomAdminView, self).get_context()
        for item in context['nav_menu']:
            if item['title'] == self.app_title:
                item['selected'] = True
                for menu in item['menus']:
                    if menu['title'] == self.menu_title:
                        menu['selected'] = True
                        break
                break
        return context
...
class ModelAdminView(CommAdminView):
    ...
    def get_breadcrumb(self):
        ...
        if self.has_view_permission() or self.has_view_permission_part():
    ...
    def get_object_url(self, obj):
        if self.has_change_permission() or self.has_change_permission_obj(obj):
            return self.model_admin_url("change", getattr(obj, self.opts.pk.attname))
        elif self.has_view_permission() or self.has_view_permission_obj(obj):
    ...
    # 添加模型的权限在原始的权限的设置，与guardian无关
    def has_add_permission(self):
        add_codename = get_permission_codename('add', self.opts)
        
        if 'add' in self.remove_permissions:
            return False
        if self.user.has_perm('%s.%s' % (self.app_label, add_codename)):
            return True
        return False
    
    # 拥有查看全部对象的权限，和guardian无关
    def has_view_permission(self):
        view_codename = get_permission_codename('view', self.opts)
        change_codename = get_permission_codename('change', self.opts)
        
        if 'view' in self.remove_permissions:
            return False
        if self.user.has_perm('%s/%s' % (self.app_label, view_codename)):
            return True
        if self.user.has_perm('%s/%s' % (self.app_label, change_codename)):
            return True
        return False
    
    # 拥有查看部分对象的权限，使用guardian进行控制
    def has_view_permission_part(self):
        view_codename = get_permission_codename('view', self.opts)
        change_codename = get_permission_codename('change', self.opts)
        
        if 'view' in self.remove_permissions:
            return False
        if get_objects_for_user(self.user, '%s.%s' % (self.app_label, view_codename)):
            return True
        if get_objects_for_user(self.user, '%s.%s' % (self.app_label, change_codename)):
            return True
        return False
    
    # 拥有查看obj对象的权限，使用guardian进行控制
    def has_view_permission_obj(self, obj):
        view_codename = get_permission_codename('view', self.opts)
        change_codename = get_permission_codename('change', self.opts)
        
        if 'view' in self.remove_permissions:
            return False
        if self.user.has_perm('%s.%s' % (self.app_label, view_codename), obj):
            return True
        if self.user.has_perm('%s.%s' % (self.app_label, change_codename), obj):
            return True
        return False
    
    # 拥有修改全部对象的权限，和guardian无关
    def has_change_permission(self):
        change_codename = get_permission_codename('change', self.opts)
        
        if 'change' in self.remove_permissions:
            return False
        if self.user.has_perm('%s.%s' % (self.app_label, change_codename)):
            return True
        return False

    # 拥有修改部分对象的权限，使用guardian进行控制
    def has_change_permission_part(self):
        change_codename = get_permission_codename('change', self.opts)
    ...
    # 拥有删除全部对象的权限，和guardian无关
    def has_delete_permission(self, request=None):
        delete_codename = get_permission_codename('delete', self.opts)
        
        if 'delete' in self.remove_permissions:
            return False
        if self.user.has_perm('%s.%s' % (self.app_label, delete_codename)):
            return True
        return False
    
    # 拥有删除部分对象的权限，使用guardian进行控制
    def has_delete_permission_part(self):
        delete_codename = get_permission_codename('delete', self.opts)
        
        if 'delete' in self.remove_permissions:
            return False
        if get_objects_for_user(self.user, '%s.%s' % (self.app_label, delete_codename)):
            return True
        return False

    # 拥有删除obj对象的权限，使用guardian进行控制
    def has_delete_permission_obj(self, obj, request=None):
        delete_codename = get_permission_codename('delete', self.opts)
        
        if 'delete' in self.remove_permissions:
            return False
        if self.user.has_perm('%s.%s' % (self.app_label, delete_codename), obj):
            return True
        return False
