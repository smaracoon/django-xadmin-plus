...
class ModelFormAdminView(ModelAdminView):
    ...
    def get_context(self):
        ...
        new_context = {
            ...
            'has_view_permission': self.has_view_permission() or self.has_view_permission_obj(self.org_obj),
            'has_change_permission': self.has_change_permission() or self.has_change_permission_obj(self.org_obj),
            'has_delete_permission': self.has_delete_permission() or self.has_delete_permission_obj(self.org_obj),
        ...
        if self.default_fields and not self.org_obj:
            model = self.model
            original = model(**self.default_fields)
            new_context.update({
                'original': original
            })
...
class CreateAdminView(ModelFormAdminView):
    ...
    def get_context(self):
        new_context = {
            ...
            'has_view_permission_guardiany': False,
        }
    ...
    def post_response(self):
        ...
        if "_addanother" in request.POST:
            ...
            elif self.has_view_permission() or self.has_view_permission_part():
...
class UpdateAdminView(ModelFormAdminView):
    ...
    def init_request(self, object_id, *args, **kwargs):
        ...
        self.old_obj = self.get_object(unquote(object_id))
    ...
    @filter_hook
    def get_context(self):
        if self.has_view_permission() or self.has_view_permission_obj(self.org_obj):
            has_view_permission_guardiany = True
            model_info = (self.opts.app_label, self.opts.model_name)
            guardiany_url = self.model_admin_url('guardiany', self.org_obj.pk)
        else:
            has_view_permission_guardiany = False
            guardiany_url = '#'
        new_context = {
            ...
            'has_view_permission_guardiany': has_view_permission_guardiany,
            'guardiany_url': guardiany_url,
        }
