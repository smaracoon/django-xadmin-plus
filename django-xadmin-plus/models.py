...
class MenuView(models.Model):
    app_label = models.CharField('应用名', max_length=256, blank=False)
    menu_label = models.CharField('页面名', max_length=256, blank=False)

    class Meta:
        verbose_name = verbose_name_plural = '自定义页'
        permissions = (
            ("visit_menuview", "can visit 自定义页"),
        )

    def __str__(self):
        return self.app_label + " 应用自定义页 " + self.menu_label
